build.db3.IBI<-function(query,refs,blast.results.file){
...
build.db4.IBI<-function(query,buffer,blast.results.file,out.final.db,obitaxo){
  ...
  message(c("Done. ", "From ", count_queries," sequences, ", count_hits, " mapped to a reference, ",count_70,
            ...
            count_left_buffer," of which passed left buffer and ",count_right_buffer,
            " passing right buffer, corresponding to ", count_final_db," unique species."))
}
