...
# colnames(all.taxatabs.ss[,-1])
# message("Samples excluded from final dataset")
# colnames(all.taxatabs[,-1])[!colnames(all.taxatabs[,-1]) %in% colnames(all.taxatabs.ss[,-1])]
...
## heatmaps 
```{r heatmaps, eval=TRUE,echo=F,fig.height=7,fig.width=20}
      ...
      for(i in 1:length(plotting.vars)){
          for(h in 1:length(values.to.plot)){
            taxatab.heatmap(taxatab = all.taxatabs.ss,master_sheet = ms_ss,group.by=plotting.vars[i],
                values=values.to.plot[h],colour.bar=colour.bar,facetcol=facetcol,inc.values = T,current.grouping = "ss_sample_id")
        }
      }
}
...
if(plot.pca) if(!is.null(plotting.vars)){
  ...
  for(j in 1:length(taxatabs.multilevels)){
    for(i in 1:length(plotting.vars)){
      print(taxatab.pca.plot.col(taxatab = taxatabs.multilevels[[j]],ms_ss = ms_ss,grouping = sumrepsby,lines = show.pca.lines,longnames = F,
                                 shortnames = T,ellipse = T,factors = plotting.vars[i],hidelegend = hidelegend.pca))
    }
  }
