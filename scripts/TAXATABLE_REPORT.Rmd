...
## heatmaps 
```{r heatmaps, eval=TRUE,echo=F,fig.height=14,fig.width=20}
      ...
      for(i in 1:length(plotting.vars)){
          for(h in 1:length(values.to.plot)){
            taxatab.heatmap(taxatab = all.taxatabs.ss,master_sheet = ms_ss,group.by=plotting.vars[i],
                values=values.to.plot[h],colour.bar=colour.bar,facetcol=facetcol,inc.values = T,current.grouping = sumrepsby)
        }
      }
}
