...
## group stats
```{r grouped stats, eval=TRUE,echo=F}
#grouping stats
#rank stats
if(!is.null(plotting.vars)){
  
  stats.list<-list()
  bigger.stats.list<-list()
  ...
  grouped.tab<-sumreps(all.taxatabs.ss,ms_ss2,grouping = plotting.vars.grouped[j],discard = F)
    ...
    for(i in 2:ncol(grouped.tab)){
      ...
      #message(colnames(xtab)[2])
      #print(stats.by.rank(taxatab = xtab))
      stats.list[[i]]<-as.data.frame(stats.by.rank(taxatab = xtab))
