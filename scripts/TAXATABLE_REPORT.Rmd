...
, rownames = FALSE, options = list(pageLength = 50, scrollX=T),caption = "Number of detections at each taxonomic rank")
   ...
   print(plot.bars2(all.stats.df[all.stats.df$name %in% unique(ms_ss2[,plotting.vars.grouped[j]]),],type = "percent.dxns"))
...
, rownames = FALSE, options = list(pageLength = 50, scrollX=T),caption = "Number of reads at each taxonomic rank")
     ...
     print(plot.bars2(all.stats.df[all.stats.df$name %in% unique(ms_ss2[,plotting.vars.grouped[j]]),],type = "percent.reads"))
  }
}
```

## pie rank stats
```{r pie rank stats, eval=TRUE,echo=F,fig.width=70}
#piechart by family
if(!is.null(plotting.vars)){
  ...
  grouped.tab<-sumreps(all.taxatabs.ss,ms_ss2,grouping = plotting.vars[j],discard = F)
  
   taxatab.pieplot.multilev(taxtab = grouped.tab,yaxisTaxlevel = "kingdom")
    taxatab.pieplot.multilev(grouped.tab,"phylum")
    taxatab.pieplot.multilev(grouped.tab,"class")
    taxatab.pieplot.multilev(grouped.tab,"order")
    taxatab.pieplot.multilev(grouped.tab,"family")
    taxatab.pieplot.multilev(grouped.tab,"genus")
  }
}
```
  ...
  message("The following plots are venns at ",names(taxatabs.multilevels)[j]," level for each of ", plotting.vars, " (if <8 dimensions)")
    ...
    for(i in 1:length(plotting.vars)){
      ...
      if(ncol(taxatabtemp)<8) {
        print(taxatab.venn.plot(taxatabtemp))
      } else {
