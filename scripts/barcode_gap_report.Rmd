...
read.and.format.full.blast<-function(input,minpident=80){
  ...
  select = c("qseqid","saccver","taxids","pident","sseq") )
    ...
    #limit to max 15 per taxid to overcome the "thunnus issue"
    fullblast_spl<-split(fullblast,fullblast$taxids)
    for(i in 1:length(fullblast_spl)){
      if(nrow(fullblast_spl[[i]])>15) fullblast_spl[[i]]<-fullblast_spl[[i]][1:15,]
    }
...
# thunnus	AY958655.1	so many seqs in this genus, that this one is not in top 100
  ...
  system2(command = "blastn",
          args=c("-query", paste0(outDir,"tempBLASTDB.fasta"), "-db",paste0(outDir,"tempBLASTDB"),"-outfmt",
                 ...
                 500, "-max_hsps",1,"-word_size", 11,"-perc_identity", 70,"-qcov_hsp_perc",98,
...
if("find_db_errors" %in% steps) {
  ...
  message("using min representatives=3")
  ...
  before<-length(unique(ax1$qseqid))
  ...
  if(length(grep("XM_.*",ax1$qseqid))>0) ax1<-ax1[-grep("XM_.*",ax1$qseqid),] 
  if(length(grep("XR_.*",ax1$qseqid))>0) ax1<-ax1[-grep("XR_.*",ax1$qseqid),] 
  if(length(grep("XP_.*",ax1$qseqid))>0) ax1<-ax1[-grep("XP_.*",ax1$qseqid),] 
  if(length(grep("XM_.*",ax1$saccver))>0) ax1<-ax1[-grep("XM_.*",ax1$saccver),] 
  if(length(grep("XR_.*",ax1$saccver))>0) ax1<-ax1[-grep("XR_.*",ax1$saccver),] 
  if(length(grep("XP_.*",ax1$saccver))>0) ax1<-ax1[-grep("XP_.*",ax1$saccver),] 
  
  message("'PREDICTED' in-silico sequences removed:",before-length(unique(ax1$qseqid)))
  removed.accs<-axtemp[!axtemp$qseqid %in% unique(ax1$qseqid),]
  removed.accs<-removed.accs[!duplicated(removed.accs$qseqid),c("qseqid","K","P","C","O","F","G","S")]
  ...
  if(!is.null(known_flags)) {
    ...
    message("Known flagged sequences removed:",before-length(unique(ax1$qseqid)))
  }
  ...
  for(h in 1:3){
    ...
    for(i in 1:length(ax.list)){
             ...
             #if(ax.list[[i]]$qseqid[1]=="MH688257.1") stop(i)
             #}
      outcome<-"failed"
      ...
      #use a threshold here. i.e. of the remaning hits, keep only >80%, justified? Already applying a 70% in blast...
      #xx<-xx[xx$pident>=80,]
      ...
      #BIG ISSUE...but I I am only passing if there are three members...so why? Actually think I can just increase tempBLAST to e.g. 500 seqs! try this  
      #if(min(xx$pident)>81) outcome<-"passed" #just using 81 to keep 80.5 etc
    ...
    for(i in 1:length(ax.list2)){
      ...
      #not implementing distant hits either, should I?
  ...
  if(length(removed.accs$qseqid)>0) allflagged<-c(removed.accs$qseqid,allflagged)
