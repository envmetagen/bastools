...
read.and.format.full.blast<-function(input,known_flags=NULL){
  
  #read full blast results
  fullblast<-data.table::fread(input,data.table=F, select = c("saccver","taxids","pident","sseq") )
...
if("selfblast" %in% steps) {
  ...
  fullblast<-read.and.format.full.blast(input) #should only do once
  ...
  if(!exists("divergence")) {
    divergence<-c(Family=5,Genus=5,Species=5)
    message("Defaulting to divergence of Family=5,Genus=5,Species=5")
  }
  ...
  argsblast=c("-query", paste0(outDir,"tempBLASTDB.fasta"), "-db",paste0(outDir,"tempBLASTDB"),"-outfmt",
...
if("find_db_errors" %in% steps) {
  ...
  if(!exists("divergence")) {
    divergence<-c(Class=5,Order=5,Family=5,Genus=5,Species=5)   ##TODO change this if extending to other levels
    message("Defaulting to divergence of Family=5,Genus=5,Species=5")
  }
  ...
  if(length(removed.accs$origseqid)>0) allflagged<-c(removed.accs$origseqid,allflagged)
...
if("calc_bar_gap" %in% steps) {
  ...
  if(!is.null(plot.limit.taxon)) {
    ...
    newtablongtemp<-newtablong[grep(plot.limit.taxon,newtablong$taxon),]
  ...
  for(h in 1:length(TaxlevelTestall)){
    message("Level=",TaxlevelTestall[h])
