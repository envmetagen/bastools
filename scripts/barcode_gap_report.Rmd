...
if("find_db_errors" %in% steps) {
  ...
  for(h in 1:3){
    ...
    for(i in 1:length(ax.list)){
             ...
             #if(ax.list[[i]]$qseqid[1]=="AB331718.1") stop(i)
             #}
      outcome<-"failed"
      ...
      #if database doesnt contain other representatives of the taxon, then pass it. If it does contain other representatives then fail it, but only if other representatives have at least one longer sequence, or  that is within 2% of query length (otherwise they wont be in blast results and can be erroneously flagged)
      if(outcome=="failed") {
        
        if(length(unique(ax[ax$origtaxleveltest==xx$origtaxleveltest[1],"qseqid"]))!=1) {
          ...
          if(max(slens)<minslen) outcome<-"passed" 
          
          # pc<-slens/qlen
          # if(pc>=1.02 | pc<=0.98) outcome<-"passed"
          # #if(max(slens)>minslen) outcome<-"failed" else outcome<-"passed"
