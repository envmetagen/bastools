...
message("input: ",input)
message("ncbiTaxDir: ",ncbiTaxDir)
message("KronaPath: ",KronaPath)
message("outDir: ",outDir)
message("steps: ",paste(steps,collapse = ","))
message("threshersteps: ",paste(threshersteps,collapse = ","))
message("plot.limit.taxon: ",plot.limit.taxon)
message("known_flags: ",known_flags)
message("TaxlevelTestall: ",paste(TaxlevelTestall,collapse = ","))
message("divergence: ",paste(divergence,collapse = ","))
message("use_flagged_accessions_bcg: ",use_flagged_accessions_bcg)
message("use_flagged_accessions_mbk: ",use_flagged_accessions_mbk)
message("rm.low.read.queries: ",rm.low.read.queries)
message("SpeciesBL: ",SpeciesBL)
message("GenusBL: ",GenusBL)
message("FamilyBL: ",FamilyBL)
message("thresher settings: 
        ")
message("plot.at.level: ",plot.at.level)
message("limit.plot.to.taxon: ",paste(limit.plot.to.taxon,collapse = ","))
message("final.table.out: ",final.table.out)
message("tops: ",paste(tops,collapse = ","))
message("pidents.list: ",paste(pidents.list,collapse = ","))
message("krona_html_db: ",krona_html_db)
message("selfblastout: ",selfblastout)
message("flagged_accessions: ",flagged_accessions)
message("flagged_error_detailed_table: ",flagged_error_detailed_table)
message("out_html: ",out_html)
message("further_potential_errors: ",further_potential_errors)

read.and.format.full.blast<-function(input,known_flags=NULL,rm.low.read.queries=0.90){
  #rm.low.read.queries means remove queries where size (in qseqid) < quantile 90% e.g. 90% of otus might have 35 reads or more
...
if("selfblast" %in% steps) {
  ...
  fullblast<-read.and.format.full.blast(input,rm.low.read.queries = rm.low.read.queries) 
  ...
  #blnket 20 word size...should modify for shorter frags
  #blanket 100 results/query, maybe not ok...but files getting too large
  ...
  argsblast=c("-task","megablast", "-query", paste0(outDir,"tempBLASTDB.fasta"), "-db",paste0(outDir,"tempBLASTDB"),"-outfmt",
                 ...
                 100, "-max_hsps",1,"-word_size", 20,"-perc_identity", perc_identity,"-qcov_hsp_perc",98,
  ...
  #remove tempdb
  file.remove(gsub("//","/",list.files(path = outDir,pattern = "tempBLASTDB.n.*",full.names = T)))
  file.remove(paste0(outDir,"tempBLASTDB.fasta"))
  file.remove(paste0(outDir,"tempBLASTDB.taxid_map.tsv"))
...
if("find_db_errors" %in% steps) {
  ...
  #now that Iḿ going to kpco levels, this is more nb
  for(h in 1:length(TaxlevelTestall)){
    ...
    } else { #coming from if nrow(sb)!=0
      message("No hits remaining after removing unknowns and sp. and crappy hits, skipping this level")
      flaggedB<-NULL
      errorsB<-NULL
      errors<-NULL
      flagged<-NULL
...
if("calc_bar_gap" %in% steps) {
  ...
  if(!is.null(plot.limit.taxon)) {
    ...
    plot.list.all[[h]]<-plot.list.loop
      ...
      message("No entries remaning after removing unknown, skipping this level") #coming from nrow(sb)!=0
      potential.errors.out[[h]]<-NULL 
      plot.list.all[[h]]<-NULL
    }
  } #end h-loop
...
```{r thresherblast, eval=TRUE,echo=F,fig.height=24,fig.width=10,results='hide'}
 #STEP4 thresher
...
for(h in 1:length(TaxlevelTest)){
  ...
  if(TaxlevelTest[h]=="O") {
    threshold.bin.blast2(df = sb,TaxlevelTest = "O",qseqidcol = "qseqid",qseqcol = "origseq",taxidcol = "origtaxid")
    sb.tbbO<-data.table::fread("tempBLASTDB.O.tsv",data.table = F)
    sb.tbbO<-add.lineage.df(sb.tbbO,ncbiTaxDir = ncbiTaxDir) #for metabin
    sb.tbbO<-merge(sb.tbbO,sbforpaths,all.x=T,all.y = F,by = "qseqid")
    sb.tbbO$origgenus<-stringr::str_split(sb.tbbO$origpath,";",simplify = T)[,6]
    sb.tbbO$origspecies<-stringr::str_split(sb.tbbO$origpath,";",simplify = T)[,7]
    sb.tbbO$origseq<-sb.tbbO$qseq
    sb.tbbO<-sb.tbbO[,c("qseqid","saccver","pident","taxids", "origpath","origseq", "K","P","C" ,"O","F","G","S","origgenus","origspecies")]
    sb.tbbO$qseqid<-gsub("_S$","_O",sb.tbbO$qseqid)
  }
...
rm(list = c("sb","sb2","sb.splS","sb.tbbG","sb.tbbF","sb.tbbO"))
...
t1<-Sys.time()
  ...
  if("threshbin" %in% threshersteps){
    ...
    bin.thresh(blast.thresh.input=gsub(".tempBLASTDB.tsv$",".tempBLASTDBALL.tsv",paste0(outDir,selfblastout))
                ,tops=tops,pidents.list=pidents.list,known_flags = known_flags,
               final.table.out = paste0(outDir,final.table.out),
...
##todo - create trace history function to trace reason why an outcome was incorrect
```
