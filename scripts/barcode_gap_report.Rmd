...
---
# The input is the results of a "thorough" blast against a large database, e.g. nt. Or it could be the results of any full length alignment (as long as the alignment has a very diverse array of taxa, and a lot of sequences)
...
## Outputs
### Plots of the binning results classified as no_hits (no hits when taxon was removed), correct (the bin taxon path matches identically the query taxon path), above (the bin taxon path was assigned at a higher taxonomic level than the one being tested), incorrect (the bin taxon was at the correct taxonomic level but was assigned to a different taxon that the original tacon path), failed (did not pass binning thresholds / lowest common ancestor was above kingdom)
...
write.table(sb2,paste0(outDir,"tempBLASTDB.ALL.tsv"),quote = F,row.names = F,sep = "\t")
...
```{r thresher, eval=TRUE,echo=F,fig.height=12,fig.width=10}
 #STEP3 calculate barcoding gaps

#split this chunk to hide loop bin output from report
...
t1<-Sys.time()
  ...
  if("threshbin" %in% threshersteps){
    ...
    sb2<-data.table::fread(paste0(outDir,"tempBLASTDB.ALL.tsv"),data.table = F)
...
for(j in 1:length(tops)){
  for(k in 1:length(pidents.list)){
    ...
    argsmbk<-c("-i",paste0(outDir,"tempBLASTDB.ALL.tsv"), "-o",paste0(outDir,binfile),"--no_mbk")
...
for(i in 1:length(outfiles)){
  a<-data.table::fread(paste0(outDir,outfiles[i],".tsv"),data.table = F)
  ...
  results[[i]]<-sb4
  file.remove(paste0(outDir,outfiles[i],".tsv"))
  file.remove(paste0(outDir,outfiles[i],".info.tsv"))
  file.remove(paste0(outDir,outfiles[i],".versions.txt"))
}
...
write.table(final.table,paste0(outDir,final.table.out),quote = F,sep = "\t",row.names = F)
...
#too long for rmd often, save to file? maybe enough to have final table 
...
#summary counts
    ...
    final.table<-data.table::fread(paste0(outDir,final.table.out),data.table = F)
