...
if("find_db_errors" %in% steps) {
  ...
  for(h in 1:3){
    ...
    for(i in 1:length(ax.list)){
             ...
             #if(ax.list[[i]]$qseqid[1]=="KC429304.1") stop(i)
            # }
      outcome<-"failed"
      ...
      if(outcome=="failed") {
        ...
        if(hutaxleveltest!=xx$origtaxleveltest[1]) if("Eukaryota;Chordata;Mammalia;Primates;Hominidae;Homo;Homo sapiens" %in% xx$hitpath) {
