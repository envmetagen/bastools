...
if("calc_bar_gap" %in% steps) {
  ...
  remove.taxa.from.list<-function(BL,sbo){
  ...
  if(!is.null(SpeciesBL)) {
    ...
    sbo<-remove.taxa.from.list(BL = SpeciesBL,sbo = sbo)
  }
  
  if(!is.null(GenusBL)) {
    message("Disabling genera")
    sbo<-remove.taxa.from.list(GenusBL,sbo)
  }
  
  if(!is.null(FamilyBL)) {
    message("Disabling families")
    sbo<-remove.taxa.from.list(FamilyBL,sbo)
  }  
...
file.remove("tempBLASTDB.F.tsv")
file.remove("tempBLASTDB.G.tsv")
...
t1<-Sys.time()
  ...
  if("threshbin" %in% threshersteps){
    ...
    bin.thresh(blast.thresh.input=gsub(".tempBLASTDB.tsv$",".tempBLASTDBALL.tsv",paste0(outDir,selfblastout))
               ...
               SpeciesBL = SpeciesBL,GenusBL = GenusBL,FamilyBL = FamilyBL)
...
if("threshplots" %in% threshersteps) {
  ...
  plot.thresh(paste0(outDir,final.table.out),limit.plot.to.taxon = limit.plot.to.taxon,plot.at.level = plot.at.level)
