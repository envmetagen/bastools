...
#keep one per saccver 22050
#keep one per sseq=29796
#keep one per staxid=9419
#keep one seq per species=8765 (think this is not ideal)
fullblast2<-fullblast[order(fullblast$pident,decreasing = T),]
fullblast2<-fullblast2[!duplicated(fullblast2$staxid),] 

#make new header names with taxids, for building db
fullblast2$seq.name<-paste0(fullblast2$saccver," taxid=", fullblast2$staxid,"; ",fullblast2$ssciname)
...
fullblast2<-add.lineage.df(fullblast2,ncbiTaxDir,taxCol = "staxid")
...
for(i in 1:length(files)){
  ...
  if(files[i]=="tempBLASTDB.S.tsv") {
    pidents.list2$Gpident<-0 
    pidents.list2$Fpident<-0
    
    settings<-expand.grid(pidents.list2)
  }
  
  if(files[i]=="tempBLASTDB.G.tsv") {
    pidents.list2$Spident<-pidents.list$Gpident #using zero doesnt work
    pidents.list2$Fpident<-0
    
    settings<-expand.grid(pidents.list2)
    settings<-settings[settings$Spident==settings$Gpident,]
  }
  ...
  if(files[i]=="tempBLASTDB.F.tsv") {
    ...
    settings<-expand.grid(pidents.list2)
    settings<-settings[settings$Spident==settings$Fpident & settings$Gpident==settings$Fpident,]
  }
  ...
  for(j in 1:length(tops)){
    for(k in 1:nrow(settings)){
      ...
      system2("metabin",c("-i",files[i], "-o",outfiles[l], "-S", settings$Spident[k],"-G", settings$Gpident[k]
                       ,"-F", settings$Fpident[k],"-A", 50, "--TopSpecies", tops[j],"--TopGenus", tops[j],"--TopFamily", tops[j]
                       ,"--no_mbk"),wait=T)
    }
...
for(i in 1:length(outfiles)){
  ...
  level<-gsub("\\.","",stringr::str_extract(outfiles[i],"\\..*\\."))
...
for(j in 1:length(unique(final.table$level))){
  ...
  for(i in 1:length(unique(final.tableS$settings))){
    ...
    countsS$correct[i]<-sum(final.tableSx[final.tableSx$settings==current.setting,paste0("correct",unique(final.table$level)[j])])
    countsS$incorrect[i]<-sum(final.tableSx[final.tableSx$settings==current.setting,paste0("incorrect",unique(final.table$level)[j])])
    countsS$above[i]<-sum(final.tableSx[final.tableSx$settings==current.setting,paste0("above",unique(final.table$level)[j])]=="above")
  }
...
allcounts$sum<-rowSums(allcounts[,c("no_hits","correct","above","incorrect")])


  
  #TODO finally, do a check if species not in db, do we get to family level
...
longcount<-reshape2::melt(allcounts[,c("no_hits","correct","above","incorrect","settings")],id.vars="settings")
...
for(i in 1:length(final.table.list)){
  ...
  longcount[longcount$value=="above","value"]<-TRUE
  longcount[(longcount$value=="species" | longcount$value=="genus" | longcount$value=="family" | 
               longcount$value=="htf"),"value"]<-FALSE
