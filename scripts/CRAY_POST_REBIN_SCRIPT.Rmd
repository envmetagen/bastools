...
## read settings
```{r, eval=TRUE,echo=F}
source("/home/tutorial/TOOLS/bastools/examples/example_POST_REBIN_SCRIPT.config.R")
...
message("Removing NAs - seqqunes that had blast hits but were not assigned to any taxon")
...
all.taxatabs.ss<-remove.contaminant.taxa(taxatab = all.taxatabs.ss,master_sheet = ms_ss,negatives = negatives,group.codes = group.codes2)
...
#taxalists
message("This needs more work, depends on collapsed taxa, need to split nto 2 tables, pre and post collapse")

taxaSteps<-data.frame(start=all.stats[[1]][2][[1]]$taxon)
rownames(taxaSteps)<-taxaSteps$start
#pre-collapse
namesSteps<-"names"
for(i in 1:length(all.stats)){
...
for(i in 1:(grep("aggregate_by_xLevel_taxa",namesSteps)-1)){
  taxaSteps[,i]<-rownames(taxaSteps) %in% all.stats[[i]][2][[1]]$taxon
}

colnames(taxaSteps)<-namesSteps[1:(grep("aggregate_by_xLevel_taxa",namesSteps)-1)]
