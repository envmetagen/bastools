...
```{r, eval=TRUE,echo=F}
source("/mnt/Disk1/BASTIAN_POST_MBC_MISEQS/POST_TAXONOMY_CHECK/SCORPION/SCORPION_POST_REBIN_SCRIPT.config.R")
  ...
  if(is.null(negatives)) {
    message("No negatives with reads, skipping contaminant filter")
      ...
      all.taxatabs.ss<-remove.contaminant.taxa(taxatab = all.taxatabs.ss,master_sheet = ms_ss,negatives = negatives,
                                               group.codes = group.codes2,remove.entire.dataset = remove.entire.dataset,rm.only.less.than=rm.only.less.than)
...
plotlist[[i]]<-taxatab.stackplot(taxatab = all.taxatabs.ss,master_sheet = ms_ss,column = plotting.vars[i],as.percent = F,as.dxns = T,hidelegend = hidelegend,grouping = sumrepsby)
}
...
for(i in 1:length(plotting.vars)){
plotlist[[i]]<-taxatab.stackplot(all.taxatabs.ss,master_sheet = ms_ss,column = plotting.vars[i],as.percent = T,as.dxns = F,hidelegend = hidelegend,grouping = sumrepsby)
}
for(i in 1:length(plotting.vars)){
  if(hidelegend==F) {
    print(plotlist[[i]])
...
for(i in 1:length(plotting.vars)){
plotlist[[i]]<-taxatab.stackplot(all.taxatabs.ss,master_sheet = ms_ss,column = plotting.vars[i],as.percent = F,as.dxns = F,hidelegend = hidelegend,grouping = sumrepsby)
}
for(i in 1:length(plotting.vars)){
  if(hidelegend==F) {
    print(plotlist[[i]])
